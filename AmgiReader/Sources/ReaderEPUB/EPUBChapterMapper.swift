...
enum EPUBChapterMapper {
    ...
    static func map(
        ...
        var chapters: [ReaderChapter] = []
        chapters.reserveCapacity(spine.items.count)
        var urls = [Int64: URL](minimumCapacity: spine.items.count)
        var totalPages = 0
        var seenIDs = Set<Int64>(minimumCapacity: spine.items.count)
