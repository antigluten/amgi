...
/// Note Manhhao: Modifications here to support modern .epub files
protocol EPUBSpineParser {
    ...
    func parse(_ xmlElement: AEXMLElement, manifestItems: [EPUBManifestItem]) -> EPUBSpine
}
    ...
    func parse(_ xmlElement: AEXMLElement, manifestItems: [EPUBManifestItem]) -> EPUBSpine {
        ...
        let items: [EPUBSpineItem] = xmlElement["itemref"].all?
            .compactMap { item in
                ...
                guard let idref = item.attributes["idref"] else { return nil }
        ...
        // EPUB3 Nav
        var tocID = manifestItems.first(where: { $0.property?.contains("nav") == true })?.id
        
        // EPUB2 Fallback
        if tocID == nil {
            tocID = xmlElement.attributes["toc"]
        }
        ...
        return EPUBSpine(
            ...
            toc: tocID,
            pageProgressionDirection: EPUBPageProgressionDirection(rawValue: direction),
            items: items
        )
    }
}
